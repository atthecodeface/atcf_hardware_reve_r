...
/*t t_reve_r_pipeline_control_fetch_action
 */
typedef enum[3] {
...
} t_reve_r_pipeline_control_fetch_action;

/*t t_reve_r_pipeline_state - early in clock cycle to affect response
 */
typedef bit[2] t_reve_r_pipeline_tag;
typedef struct {
    ...
    t_reve_r_mode mode "Mode the pipeline is executing in";
    bit          error;
    t_reve_r_pipeline_tag tag;
    ...
    t_reve_r_mode interrupt_to_mode "If interrupt then this is the mode that whose pp/pie/epc should be set from current mode's";
    bit[32]           instruction_data;
    t_reve_r_inst_debug instruction_debug;
} t_reve_r_pipeline_state;

/*t t_reve_r_pipeline_control_decode - late in clock cycle to affect clocking
 */
typedef struct {
...
} t_reve_r_pipeline_control_decode;

/*t t_reve_r_pipeline_control_exec - late in clock cycle to affect clocking
 */
typedef struct {
...
//    bit ret "Asserted for RET instructions - presumably needs an indication as to which mode to return to";
    ...
    bit[32] pc_if_mispredicted "Target for a mispredicted branch - used in the state";
} t_reve_r_pipeline_control_exec;

/*t t_reve_r_pipeline_control_exec - late in clock cycle to affect clocking
 */
typedef struct {
    bit blocked           "Asserted if mem stage is valid AND is not completing; if low, mem can take from exec (unless flushing exec)";
} t_reve_r_pipeline_control_mem;

/*t t_reve_r_pipeline_control_flush - late in clock cycle to affect clocking
 */
typedef struct {
    bit fetch;
    bit decode;
...
} t_reve_r_pipeline_control_flush;

/*t t_reve_r_pipeline_control - late in clock cycle to affect clocking
 */
typedef struct {
    t_reve_r_i32_trap trap;
    t_reve_r_pipeline_control_flush   flush;
    t_reve_r_pipeline_control_decode  decode;
    t_reve_r_pipeline_control_exec    exec;
    t_reve_r_pipeline_control_mem     mem;
} t_reve_r_pipeline_control;

/*t t_reve_r_pipeline_response_decode
 */
typedef struct {
    ...
    t_reve_r_decode idecode "Decode of instruction (if valid)";
    bit enable_branch_prediction "Asserted if branch prediction (and hence branch_target) is to be used";
} t_reve_r_pipeline_response_decode;

/*t t_reve_r_pipeline_response_exec
 */
typedef struct {
    bit valid;
    ...
    bit[32] pc_if_mispredicted "From pipeline_fetch_data associated with the decode of this instruction";
    bit branch_condition_met;
    t_reve_r_dmem_access_req dmem_access_req;
    t_reve_r_csr_access     csr_access;
} t_reve_r_pipeline_response_exec;

/*t t_reve_r_pipeline_response_mem
 */
typedef struct {
    bit valid;
...
} t_reve_r_pipeline_response_mem;

/*t t_reve_r_pipeline_response_rfw
 */
typedef struct {
    bit valid;
    bit rd_written;
    bit[5] rd;
    bit[32] data;
} t_reve_r_pipeline_response_rfw;

/*t t_reve_r_pipeline_response - mid cycle (dependent on control) to effect fetch request and hence fetch data
 */
typedef struct {
    t_reve_r_pipeline_response_decode decode;
    t_reve_r_pipeline_response_exec   exec;
    t_reve_r_pipeline_response_mem    mem;
    t_reve_r_pipeline_response_rfw    rfw;
    bit                              pipeline_empty;
} t_reve_r_pipeline_response;

/*t t_reve_r_pipeline_trap_request
...
} t_reve_r_pipeline_trap_request;

/*t t_reve_r_pipeline_fetch_req
 */
typedef struct {
...
} t_reve_r_pipeline_fetch_req;

/*t t_reve_r_pipeline_fetch_data
 */
typedef struct {
    bit          valid;
    t_reve_r_mode mode "Mode the pipeline is executing in";
    ...
    bit[32] dec_pc_if_mispredicted;
} t_reve_r_pipeline_fetch_data;

/*t t_reve_r_pipeline_debug_control
 */
typedef struct {
    bit valid;
    bit kill_fetch;
...
} t_reve_r_pipeline_debug_control;

/*t t_reve_r_pipeline_debug_response
 */
typedef struct {
    bit exec_valid;
    bit exec_halting;
    bit exec_dret;
} t_reve_r_pipeline_debug_response;
